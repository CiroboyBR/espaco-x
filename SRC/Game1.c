...
int main() {
    ...
    while (!key[KEY_ESC]) {
        ...
        if ( (double)(clock() - tempo) >= 1000/FPS ) {
            ...
            char txt[15];
            sprintf(txt,"x + %d = %d", b, a);			//Inicializa o texto
            char txt2[15];
            sprintf(txt2,"Pontos = %d", score);		//Inicializa o texto
            ...
            draw_sprite(buffer, background[back], 0, 0);			//Coloca a imagem na tela
            draw_sprite(buffer, nave[e], xnave[e],ynave[e]);     	//Põe a nave na tela
            textout(buffer,font,txt,600,450,makecol(255,0,0));  	//Exibe o texto
            textout(buffer,font,txt2,5,5,makecol(255,0,0));  		//Exibe o texto
            draw_sprite(buffer, inimigo[p], 15,yini); 				//Põe a nave na tela
...
void atirar() {
    ...
    while (tiro.vida <= 19) {
        ...
        if (tiro.py > yini + 125) {
            ...
            draw_sprite(buffer, inimigo[p], xini, yini); 				//Põe a nave na tela
...
void colisao() {
    ...
    while (!key[KEY_ENTER]) {
        readkey();
        textout(buffer, font, "Acertou a nave desprotegida! Tecle 'Enter' para continuar", 400,350, makecol(255,0,0 ) );
        char txt[15];
        sprintf(txt,"x + %d = %d", b, a);				//Inicializa o texto
        char txt2[15];
        sprintf(txt2,"Pontos = %d", score);			//Inicializa o texto
        ...
        if (score == 300) {
            nivel = 2.0;
            textout(buffer, font, "Nivel 2", 600,365, makecol(255,0,0 ) );
            back = 1;
        }
        if (score == 500) {
            nivel = 4.0;
            textout(buffer, font, "Nivel 3", 600,365, makecol(255,0,0 ) );
            back = 2;
        }
        if (score == 600) {
            nivel = 5.0;
            textout(buffer, font, "Nivel 4", 600,365, makecol(255,0,0 ) );
            back = 3;
        }
        if (score == 700) {
            nivel = 6.0;
            textout(buffer, font, "Nivel 5", 600,365, makecol(255,0,0 ) );
            back = 4;
        }
        if (score == 800) {
            nivel = 10.0;
            textout(buffer, font, "Nivel 6", 600,365, makecol(255,0,0 ) );
            back = 5;
        }
        if (score == 900) {
            nivel = 16.0;
            textout(buffer, font, "Nivel 7", 600,365, makecol(255,0,0 ) );
            back = 6;
        }
        if (score == 1000) {
            nivel = 20.0;
            textout(buffer, font, "Nivel MAX", 600,365, makecol(255,0,0 ) );
            back = 7;
        }
    }
    blit(buffer, screen, 0,0,0,0,1280,720);     					//Limpa a tela
...
void perdeu() {
    ...
    while (!key[KEY_ENTER]) {
        ...
        textout(buffer, font, "Voce perdeu! Tecle 'Enter' para comecar novamente", 450,350, makecol(255,0,0 ) );
        char txt[15];
        sprintf(txt,"x + %d = %d", b, a);				//Inicializa o texto
        char txt2[15];
        sprintf(txt2,"Score = %d", score);			//Inicializa o texto
        textout(buffer,font,txt,5,5,makecol(255,0,0));  			//Exibe o texto
        textout(buffer,font,txt2,5,20,makecol(255,0,0));  			//Exibe o texto
        if ( (double)(clock() - tempo) >= 1000/FPS ) {
            if (p == 10) {
                p++;
            }
                    ...
                    if (p == 12) {
                        ...
                        if (p == 13) {
                            p = 10;
                              			...
                              			//Limpa a tela
            draw_sprite(buffer, background[back], 0, 0);      		//Coloca a imagem na tela
            draw_sprite(buffer, bomba[m], x, y);
            draw_sprite(buffer, inimigo[p], 15,yini); 				//Põe a nave na tela
             blit(buffer, screen, 0,0,0,0,1280,720);     			//Limpa a tela
            clear_bitmap(buffer);
            m++;
...
void explosao2() {
    ...
    while(m < 10) {
        if ( (double)(clock() - tempo) >= 1000/FPS ) {
            ...
            draw_sprite(buffer, background[back], 0, 0);      		//Coloca a imagem na tela
            draw_sprite(buffer, bomba[m], -50, y);
            draw_sprite(buffer, bomba[m], 75, y);
            draw_sprite(buffer, bomba[m], 200, y);
            draw_sprite(buffer, bomba[m], 325, y);
            draw_sprite(buffer, bomba[m], 450, y);
            draw_sprite(buffer, bomba[m], 575, y);
            draw_sprite(buffer, bomba[m], 700, y);
            draw_sprite(buffer, bomba[m], 825, y);
            draw_sprite(buffer, bomba[m], 950, y);
            draw_sprite(buffer, bomba[m], 1075, y);
            draw_sprite(buffer, nave[e], xnave[e],ynave[e]);     	//Põe a nave na tela
             blit(buffer, screen, 0,0,0,0,1280,720);     			//Limpa a tela
            clear_bitmap(buffer);                       			//Limpa a tela
            m++;
